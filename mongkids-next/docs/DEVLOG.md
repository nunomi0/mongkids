...
## [2026-03-14] 세션 26: 체험 예약과 학생 레코드 영구 연결

### 작업 내용

1. **체험 예약 연결 로직 구현**
   - `types/student.ts`에 `TrialReservation.student_id` 추가
   - `lib/queries/trials.ts`에 `linkTrialToStudent()` 추가
   - 체험 예약 등록 완료 후 `trial_reservations.student_id`를 저장하도록 연결

2. **체험 등록 모달 흐름 보강**
   - `trial-detail-modal.tsx`에서 등록 전 상태와 등록 후 상태를 함께 전달하도록 수정
   - `trials-client.tsx`에서 학생 등록 성공 시 체험 예약과 학생을 연결
   - 학생 등록 모달을 취소하면 체험 상태를 이전 값으로 되돌리도록 처리

3. **학생 등록 모달 저장 흐름 보정**
   - `add-student-modal.tsx`의 `onSaved`를 비동기 저장 결과를 기다리도록 수정
   - 저장 실패 시 모달이 닫히지 않도록 반환값 기반 제어 추가
   - 학생 목록 페이지와 체험 페이지의 저장 핸들러를 boolean 반환으로 정리

4. **문서 갱신**
   - `OPERATIONS_POLICY`, `FEATURES`, `CLAUDE` 갱신

### 의사결정

- 체험 예약의 `등록` 상태는 학생 생성과 함께 끝나야 의미가 있으므로 `student_id`까지 저장
- 학생 등록 모달을 닫기만 한 경우는 등록 완료로 보지 않고 이전 체험 상태로 복원
- 학생 등록 모달 저장은 비동기 작업 완료 후 닫히도록 맞춰 레이스를 제거

---

## [2026-03-12] 세션 25: 결제·레벨·시간표 정책 반영

### 작업 내용

1. **정리 작업**
   - `tmp_memo_for_database.txt` 삭제
   - 비어 있던 `hooks/` 디렉터리 제거

2. **결제 중복 정책 반영**
   - 같은 학생의 같은 `target_month` 결제를 여러 건 허용하도록 엑셀 업로드 로직 수정
   - 대신 동일 결제일·월·금액·메모·할인 조합은 재업로드 시 중복 생성하지 않도록 조정

3. **레벨 계산 기준 수정**
   - `lib/queries/levels.ts`의 현재 레벨 계산을 최고 급수 기준에서 가장 최근 취득일 기준으로 변경

4. **시간표 수정 동기화 구현**
   - `lib/queries/students.ts`에 예정 출석 재조정 로직 추가
   - 학생 시간표 수정 시 미래 `예정` 출석만 삭제 후 새 시간표 기준으로 다시 생성
   - `출석`, `결석`, `보강예정`, `보강완료`는 유지

5. **문서 갱신**
   - `docs/OPERATIONS_POLICY.md`에 같은 월 추가 결제 정책 반영
   - `docs/FEATURES.md`에 쿼리 모듈 분리, 시간표 재조정, 결제 중복 허용 로직 반영
   - `CLAUDE.md`를 현재 구조와 운영 정책 기준에 맞게 재정리

### 의사결정

- 같은 월 결제는 4주 기준 정규 결제 외 5주차 수업, 특강, 추가 등록을 위해 허용
- 엑셀 업로드는 같은 월 전체 스킵이 아니라 "완전히 같은 결제만 스킵" 기준으로 관리
- 시간표 수정은 미래 `예정` 출석만 다시 맞추고, 이미 운영 이력이 생긴 출석 기록은 보존

---

## [2026-03-12] 세션 24: 훅 제거와 쿼리 구조 분리

### 작업 내용

1. **모바일 전용 훅 제거**
   - `hooks/use-mobile.tsx` 삭제
   - `components/ui/sidebar.tsx`를 데스크톱 전용 사이드바 구조로 단순화

2. **쿼리 파일 분리**
   - `lib/queries.ts`의 구현을 `lib/queries/` 하위 도메인 파일로 분리
   - 학생, 결제, 레벨, 수업, 체험, 대시보드, 엑셀 단위로 정리
   - `lib/queries.ts`는 re-export 진입점으로 축소

### 의사결정

- 모바일 대응이 현재 프로젝트 범위가 아니므로 모바일 판별 훅은 제거
- 화면 import 경로는 유지하고, 내부 구조만 도메인 단위로 분리
- `queries` 명칭은 현재 역할과 크게 어긋나지 않으므로 유지
