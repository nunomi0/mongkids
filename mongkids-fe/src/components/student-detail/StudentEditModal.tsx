          ...
          <div className="flex flex-col gap-4">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-3 p-4 pt-2">
              ...
              <div>
                ...
                <Input type="date" value={form.birth_date} onChange={(e)=>setForm(f=>f?{...f, birth_date: e.target.value}:f)} />
              </div>
              <div>
              ...
              <div>
                ...
                <Input placeholder="mm" value={form.shoe_size} onChange={(e)=>setForm(f=>f?{...f, shoe_size: e.target.value}:f)} />
              </div>
              <div>
            ...
            <div className="px-4">
              ...
              {scheduleValidation.hasDuplicate && (
                <div className="text-xs text-red-500 mb-2">수업 시간이 중복되었습니다. 요일/시간/그룹이 동일한 항목이 있습니다.</div>
              )}
              {scheduleValidation.countMismatch && (
                <div className="text-xs text-red-500 mb-2">현재 선택한 반은 주 {scheduleValidation.required}회 수업입니다. 시간 수가 일치하지 않습니다.</div>
              )}
            ...
            <div className="flex items-center justify-between p-4 pt-2">
              <Button variant="outline" className="text-red-600" onClick={()=>{ setConfirmName(''); setDeleteOpen(true) }}>학생 삭제</Button>
              <div className="flex items-center gap-2">
                <Button variant="outline" onClick={onClose}>취소</Button>
                <Button onClick={handleSave} disabled={scheduleValidation.hasDuplicate || scheduleValidation.countMismatch} title={(scheduleValidation.hasDuplicate||scheduleValidation.countMismatch)?'수업 시간 검증 오류로 저장할 수 없습니다.':''}>저장</Button>
              </div>
            </div>
          </div>
        )}
